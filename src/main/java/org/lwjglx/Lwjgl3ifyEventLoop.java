...
public class Lwjgl3ifyEventLoop {
        ...
        while (SDL_PollEvent(event)) {
            ...
            switch (event.type()) {
                case SDL_EVENT_MOUSE_MOTION -> {
                    if (ForgeEarlyConfig.DEBUG_PRINT_MOUSE_EVENTS) {
                ...
                case SDL_EVENT_MOUSE_WHEEL -> {
                    ...
                    Mouse.addWheelEvent(yoffset == 0 ? (ForgeEarlyConfig.INPUT_INVERT_WHEEL ? -xoffset : xoffset) : yoffset);
                }
                case SDL_EVENT_KEYMAP_CHANGED -> {
                    LWJGLUtil.LOGGER.info("System keyboard layout changed, reloading key names");
    ...
    private static void handleTextEvent() {
        ...
        if (ForgeEarlyConfig.DEBUG_PRINT_KEY_EVENTS) {
            LWJGLUtil.LOGGER.info("[DEBUG-KEY] ns:{} text:<{}>", ns, text);
        }
